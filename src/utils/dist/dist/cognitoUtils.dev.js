...
var app_config_json_1 = require('../config/app-config.json'); // import AWS from 'aws-sdk';


aws_sdk_1.config.region = app_config_json_1['default'].AWS_COGNITO_CONFIG.REGION; // Creates a CognitoAuth instance
...
var createCognitoAuth = function createCognitoAuth() {
  ...
  var auth = new amazon_cognito_auth_js_1.CognitoAuth({
    ...
    RedirectUriSignIn: app_config_json_1['default'].AWS_COGNITO_CONFIG.CALLBACK_URI,
...
var getCognitoSignInUri = function getCognitoSignInUri() {
  var signinUri = app_config_json_1['default'].AWS_COGNITO_CONFIG.USER_POOL_BASE_URI + '/login?response_type=code&client_id=' + app_config_json_1['default'].AWS_COGNITO_CONFIG.CLIENT_ID + '&redirect_uri=' + app_config_json_1['default'].AWS_COGNITO_CONFIG.CALLBACK_URI;
  return signinUri;
}; // Parse the response from a Cognito callback URI (assumed a token or code is in the supplied href). Returns a promise.


var parseCognitoWebResponse = function parseCognitoWebResponse(href) {
  return new Promise(function (resolve, reject) {
    var auth = createCognitoAuth(); // userHandler will trigger the promise
...
var getCognitoSession = function getCognitoSession() {
  return new Promise(function (resolve, reject) {
    var cognitoUser = createCognitoUser();
    cognitoUser && cognitoUser.getSession(function (err, result) {
      if (err || !result) {
...
var signOutCognitoSession = function signOutCognitoSession() {
